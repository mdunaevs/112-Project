...
class Btn(object):
    ...
    def __init__(self, imageFile, xPos, yPos, size):
    ...
    def buttonClicked(self, xClicked, yClicked):
        leftBound = self.xPos - self.size // 2
        rightBound = self.xPos + self.size // 2
        topBound = self.yPos - self.size // 2
        botBound = self.yPos + self.size // 2
        if(leftBound <= xClicked <= rigthBound and \
...
class NewCardBtn(Btn):
    ...
    def buttonAction(self, playerCards, d1, d2):
        randDeck = math.random(0, 1)
        if(randDeck == 0):
            playerCards.append(d1.getRandomCard())
        if(randDeck == 1):
            playerCards.append(d2.getRandomCard())
...
class EndTurnBtn(Btn):
    ...
    def __init__(self, imageFile, xPos, yPos, size):
        ...
        self.imageFile = imageFile
        self.img = Image.open(self.imageFile)
        self.size = max(Card.cardWidth, Card.cardHeight)
